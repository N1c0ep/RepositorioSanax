 ...
 * @author dlpol
 */
@WebServlet(name = "ConsultarPacientesVacunadosFase", urlPatterns = {"/ConsultarPacientesVacunadosFase"})
public class ConsultarPacientesVacunadosFase extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (request.getParameter("action").equals("Buscar")) {
                ...
                for (int i = 0; i < listaCita.size(); i++) {
                    ...
                    if(listaCita.get(i).getIdentificacion().getDosis()==2 && listaCita.get(i).getFase().equals(fase)){
                        listaUsers.add(listaCita.get(i).getIdentificacion());
                    }
                }
                request.setAttribute("rows", listaUsers);
                    request.getRequestDispatcher("ConsultarPacientesVacunadosFase.jsp").forward(request, response);
